...
If there are no subturf data, then you can use `make_turf_plot_no_subturfs()`. It is essentially a wrapper for `ggplot2::geom_tile()`.
...
heath |>
  select(-starts_with("freq")) |> # remove subturf data
  (\(x){
    make_turf_plot_no_subturfs(
      data = x,
